...
<body>
	...
	<div class="page-container">
		...
		<div class="site-header">
			...
			<div class="top-menu">
				<a class="menu-button" href="FacilityPageServlet">施設情報を管理</a> <a
					class="menu-button" href="MessageListServlet"> メッセージ確認 <span
					class="unread-badge">未読<c:out value="${facilityUnreadCount}" />件
				</span>
				</a> <a class="menu-button logout-button" href="LogoutServlet">ログアウト</a>
			</div>
		...
		<div class="facility-dashboard-card">
			...
			<c:if test="${not empty sessionScope.facilityId}">
				...
				<div class="dashboard-title-area">
					<h1>施設ダッシュボード</h1>
					<p class="welcome-message">予約・メッセージ・登録ペット・アクセス状況を確認できます。</p>
				</div>
					...
					<div class="dashboard-status-panel">
						...
						<div class="status-row">
							<div class="status-row-main">
								<span class="status-icon">📅</span> <span class="status-title">予約対応</span>
							</div>
						...
						<div class="status-row">
							<div class="status-row-main">
								<span class="status-icon">💬</span> <span class="status-title">メッセージ対応</span>
							</div>
							<div class="status-value">
								<c:out value="${facilityUnreadCount}" />
								件
						...
						<div class="status-row">
							<div class="status-row-main">
								<span class="status-icon">🐾</span> <span class="status-title">ペット掲載</span>
							</div>
							<div class="status-value">
								<c:out value="${petCount}" />
								匹
							...
							<c:when test="${not empty reserve}">
								<div class="next-reserve-timeline">
									...
									<div class="timeline-content">
										<p>
											<span class="pet-label">予約ID：</span>
											<c:out value="${reserve.reservationID}" />
										</p>
										<p>
											<span class="pet-label">ペットID：</span>
											<c:out value="${reserve.petID}" />
										</p>
										<p>
											<span class="pet-label">ユーザーID：</span>
											<c:out value="${reserve.userID}" />
										</p>
										<p>
											<span class="pet-label">予約日時：</span>
											<c:out value="${reserve.formattedReserveTime}" />
										</p>
									</div>
								</div>
							</c:when>
						...
						<c:choose>
							<c:when test="${not empty latestPet}">
								<div class="latest-pet-box">
									...
									<img class="latest-pet-image" src="${latestPet.imagePath}"
										width="200">
						...
						<div class="access-visual-box">
							<div class="access-ring">
								<div class="access-ring-inner">
									<span><c:out value="${viewCount}" /></span> <small>回</small>
								</div>
							</div>
